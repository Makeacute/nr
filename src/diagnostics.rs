...
pub fn run_doctor(config: &NrConfig) -> crate::errors::Result<i32> {
    ...
    if config.remote.target_host.is_some() || config.remote.build_host.is_some() {
        if command_exists("ssh") {
            println!("  ✓ ssh: {}", tool_version("ssh"));
        } else {
            missing_required.push("ssh");
            println!("  ✗ ssh: not installed");
        }
    }
    for tool in OPTIONAL_TOOLS {
        if command_exists(tool) {
            ...
            println!("  ✗ {tool}: not installed (optional)");
        }
    }
    ...
    if summary.repository {
        ...
        if summary.dirty {
            println!("  ⚠ git: dirty working tree");
        } else {
    ...
    println!(
        "  {} user config: {}",
        if config.user_config_path.is_some() {
            "✓"
        } else {
            "⚠"
        },
    ...
    println!(
        "  {} repo config: {}",
        if config.repo_config_path.is_some() {
            "✓"
        } else {
            "⚠"
        },
    ...
    println!(
        "  ✓ retention: logs={} reports={} history={} plans={}",
    ...
    {
        Ok(()) => println!("  ✓ state dir writable: {}", state.display()),
        Err(error) => println!("  ✗ state dir writable: {} ({error})", state.display()),
    }
    ...
    ) && output.code == 0
    {
        ...
        println!("  ⚠ flakes: could not inspect experimental-features");
    }
